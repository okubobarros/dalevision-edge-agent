...
Copy-Item .\release\01_CONFIGURAR.bat .\release\win\01_CONFIGURAR.bat -Force
Copy-Item .\release\02_TESTE_RAPIDO.bat .\release\win\02_TESTE_RAPIDO.bat -Force
Copy-Item .\release\03_INICIAR.bat .\release\win\03_INICIAR.bat -Force
Copy-Item .\release\04_DIAGNOSTICO.bat .\release\win\04_DIAGNOSTICO.bat -Force
Copy-Item .\scripts\install-service.ps1 ".\release\win\05_INSTALAR_AUTOINICIO.ps1" -Force
Copy-Item .\scripts\verify-service.ps1 ".\release\win\06_VERIFICAR_AUTOINICIO.ps1" -Force
...
# 7) sanity check
python -c "import zipfile; z=zipfile.ZipFile('$zipName'); names=[i.filename for i in z.infolist()]; required={'DaleVision Edge Agent.exe','01_CONFIGURAR.bat','02_TESTE_RAPIDO.bat','03_INICIAR.bat','04_DIAGNOSTICO.bat','05_INSTALAR_AUTOINICIO.ps1','06_VERIFICAR_AUTOINICIO.ps1','README.txt','.env'}; missing=required-set(names); assert not missing, f'Missing {missing}'; assert '.env.example' not in names, 'Found .env.example in ZIP'; print('ZIP_OK files=', names)"
